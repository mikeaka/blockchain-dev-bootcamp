...
contract('Exchange', ([deployer, feeAccount, user1, user2 ]) => {
	...
	describe('withdraw Ether', async () => {
		...
		describe('success', async () => {
			...
			it('ether withdraw Ether funds', async () => {
				const balance = await exchange.tokens(ETHER_ADDRESS, user1)
	...
	describe('withdraw tokens', async () => {
		...
		describe('success', async () => {
			...
			it('Emit token Withdraw event', async () => {
				const log = result.logs[0]
				log.event.should.eq('Withdrawn')
				const event = log.args
				event.token.toString().should.equal(token.address, 'token is correct')
	...
	describe('make orders', async ()=> {
		...
		it('tracks the newly created order', async () => {
			...
			order.amountGet.toString().should.equal(tokens(1).toString(), 'amountGet amount is correct')
		...
		it('Emit an Order event', async () => {
			...
			event.amountGet.toString().should.equal(tokens(1).toString(), 'amountGet amount is correct')
